...
async function main() {
  ...
  try {
    ...
    const filterName = '강인혁';
    const clicked = await page.evaluate((name) => {
      // 사이드바에서 텍스트가 정확히 일치하는 요소 찾기
      const all = Array.from(document.querySelectorAll(
        'li, a, span, div, button, [class*="menu"], [class*="folder"], [class*="drawer"], [class*="item"]'
      ));
      const target = all.find(el =>
        el.children.length === 0 && (el.innerText || '').trim() === name
      );
      if (target) {
        target.click();
        return true;
    ...
    if (clicked) {
      console.log('✅ 강인혁 필터 클릭 성공');
      await sleep(2500);
      // 필터 적용 후 티켓 목록 재렌더링 대기
      try {
        await page.waitForFunction(
          () => /\b\d{4,6}\b/.test(document.body.innerText),
      ...
      console.log('⚠️ 강인혁 필터를 찾지 못함 — 현재 페이지 그대로 진행');
    }
